...
def searchsquare(board, playerXY):  #defines a function
    boaard = checkBlank(board, playerXY)  #use an outside function to do something
    return(boaard)
...
def gamefunc(playerXY, board, coins, gridsize):  #do this forever
    while True:
        ...
        while dier == True:
            if move != " ":
                try:
                    dist = int(input("How far do you want to move in that direction?: ")) #asks the user how far they want to move
                    dier = False
                except ValueError:
        ...
        if move == "w":  #if the user inputs w
            ...
            if playerXY[0]+dist > 7:  #if the players X is too big
                ...
                playerXY = playerXY[0]+dist, playerXY[1]  #move the player
##        elif move == " ":  #if the user inputs space
##            result = "\\:#~ERR.EXE IS NOT RESPONDING~#://"
##            result = searchsquare(board, playerXY)  #run the searchsquare function
##            print("You search the square...")  #Tells the user they search the square
##            print("You find a", result)
